...
llamadaExpr
    : ID PA listaArgumentos? PC
    ;

// Bloque de código
cuerpo
    : LA (instruccion | nota)* LC
    ;
...
condicionIf
    : IF PA expresion PC cuerpo (ELSE cuerpo)?
    ;

bucleWhile
    : WHILE PA expresion PC cuerpo
    ;
...
definicionFuncion
    : tipoDato ID PA listaParams? PC cuerpo
    ;
...
definicionVariable
    : (CONST)? tipoDato ID (IGUAL expresion)? PYC
    ;

definicionSinPunto
    : (CONST)? tipoDato ID (IGUAL expresion)?
    ;
...
FECHAISO : DIGITO DIGITO DIGITO DIGITO '/' ('0' [1-9] | '1' [0-2]) '/' ('0' [1-9] | '1' [0-9] | '2' [0-9] | '3' [01]) ;
...
PASSWORD : MAYUSCULA (LETRA | DIGITO | SIMBOLO)+ (LETRA | DIGITO | SIMBOLO)+ (LETRA | DIGITO | SIMBOLO)+ (LETRA | DIGITO | SIMBOLO)+ (LETRA | DIGITO | SIMBOLO)+ ;
NOMBRE_APELLIDO : MAYUSCULA LETRA_MINUSCULA+ ' ' MAYUSCULA LETRA_MINUSCULA+ ;
PATENTE : LETRA LETRA LETRA ' ' DIGITO DIGITO DIGITO | LETRA LETRA ' ' DIGITO DIGITO LETRA LETRA ;
