...
public class SimbolosListener extends MiLenguajeBaseListener {
    ...
    public List<String> getErrores()       { return listaErrores; }
    public List<String> getAdvertencias()  { return listaAvisos;   }
    ...
    public void enterBucleFor(MiLenguajeParser.BucleForContext ctx) {
        ...
        if (init != null && init.definicionSinPunto() != null) {
            ...
            if (dv.expresion() != null) varSym.marcarComoInicializada();
            if (dv.CONST() != null)    varSym.setConstante(true);
    ...
    public void enterDefinicionVariable(MiLenguajeParser.DefinicionVariableContext ctx) {
        ...
        if (ctx.expresion() != null) vs.marcarComoInicializada();
        if (ctx.CONST() != null)    vs.setConstante(true);
    }
    ...
    public void enterInicio(MiLenguajeParser.InicioContext ctx) {
        // Revisa existencia de main y una sola notificación por sentencias globales
        boolean foundMain = false;
        boolean flagged   = false;
        for (ParseTree ch : ctx.children) {
            if (ch instanceof MiLenguajeParser.InstruccionContext) {
                var inst = (MiLenguajeParser.InstruccionContext) ch;
                if (inst.definicionFuncion() != null && "main".equals(inst.definicionFuncion().ID().getText())) {
                    foundMain = true;
                } else if (!flagged) {
                    int ln = inst.getStart().getLine();
                    listaErrores.add("[SEMANTIC] Línea " + ln + ": solo funciones en nivel raíz.");
                    flagged = true;
                }
            }
        }
